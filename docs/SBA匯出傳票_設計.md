...
### 2.1 分組（使用者規則：**專戶 × 付款方式**，最多 6 種）
- **單據類型（動支單/請購單）不作為分組依據**。
- 分組維度＝`acctCode`（11010202 保管款／11010201 醫療基金）× 付款類別：
  | 類別 | 判定 | SBA payway |
  |---|---|---|
  | 農會存帳 | 銀行代碼 ∈ {6180069 佳里區農會, 6180416 佳興分部}（或行名含「佳里區農會」） | `1` 存帳 |
  | 電匯 | 其他銀行且有帳號 | `6` 電匯(e企) |
  | 繳費單代繳 | payee.type = 'bill' | `2` 自領（SBA 無「代繳」碼） |
- 2 專戶 × 3 類別 → **最多 6 張支出傳票**；可切換「每單獨立一張」。
- **一單跨類別**（如同單有農會員工與郵局員工）：依各受款人金額**拆入不同傳票**，借方金額同步拆分，
  確保每張傳票付款方式單一（Σ拆分＝原單金額，會提示已拆分）。
