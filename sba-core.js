...
function validateVoucher(v, opt) {
  ...
  lines.forEach((L) => {
    ...
    /* v5.7.21 實測：立沖/暫付科目帶用途別 → SBA 報「用途別科目 210205-XXXX 未設定於最細層級
       會計科目設定」整張退（SBA 該類科目根本無用途別欄）→ 工坊端提前擋 */
    if (/^(2102|1807|2807)/.test(String(L.code)) && L.use)
 ...
 *  v5.7.30 依財政部 112 年新制改「可被 5 整除」（舊制除 10；新統編僅符合新制，舊統編不受影響）。 */
function isValidGui(no) {
  ...
  return sum % 5 === 0 || (no[6] === '7' && (sum + 1) % 5 === 0);
}
/**
