/* POST /api/gemini — 傳票摘要 AI 潤飾中繼（需 X-Admin-Pass）
   ...
   規則（v5.8.7）：每段輸出必含民國年月(或年月日)+金額元；memo=年月+各名目含金額；lines/payees text=「年月+對象+費用+金額」 */
const CORS = {
...
export async function onRequestPost({ request, env }) {
  ...
  const prompt = '你是台灣佳里區衛生所醫療作業基金的會計。以下每張支出傳票有「主檔摘要草稿 main」與「明細列 lines（text=摘要草稿、amt=該列明細金額）」，草稿常過長、有公文贅詞。請依本所開票習慣改寫：\n'
    + '【memo 傳票主檔摘要】開頭必為民國年月（如 115.8），列舉本張各明細的費用大類別（頓號串接），每個名目後帶其金額（千分位+元），'
    + '如「115.8行動醫院郵資6,586元、印刷費1,478元、誤餐費1,600元」；'
    + '以 45 個中文字為上限（SBA 欄位限制），名目太多放不下時保留主要名目＋金額、其餘以「等費用，總計N元」收斂，不可只寫籠統的「雜項支出」。\n'
    + '【lines[].text 明細摘要】核心格式＝「年月（或年月日）＋對象（公司/廠商/人）＋什麼費用＋多少錢」，其餘一律省略；'
    + '每列文字必須含民國年月（或年月日）；必須含該列 amt 的金額且與 amt 完全一致，金額一律千分位加「元」（1039→1,039元）。'
    ...
    + '共同規則：每段輸出文字至少都要含「民國年月或年月日」與「金額元」，名目才清楚；繁體中文；年月縮寫（115.7／11507／115年7月）；'
